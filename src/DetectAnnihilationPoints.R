...
if(length(args <- commandArgs(trailingOnly = TRUE)) == 0){
  cat("\nUsage: ")
  cat("Rscript [--vanilla] DetectAnnihilationPoints.R <parameters.json> \n\n")
...
require("data.table", quietly = TRUE) # (data must fit in RAM)
...
# --- MAIN FUNCTION ---
# remaps hits onto centres of strips, estimates annihilation points and saves it to file
# validate_hits and progress_info could be changed here (the latter slows the calculation!)
ann_pts_environment$estimateAndSaveAnnPoints(validate_hits = FALSE, 
                                             progress_info = TRUE)
